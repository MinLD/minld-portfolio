...
test('anonymous user can create and list visible moment comments', async () => {
  ...
  const created = await request(app).post(`/api/v1/moments/${published.id}/comments`).send({ authorName: 'Guest', content: 'Nice moment' })
  expect(created.status).toBe(201)
  ...
  expect((await request(app).post(`/api/v1/moments/${published.id}/comments`).send({ content: 'No name' })).status).toBe(400)
  expect((await request(app).post(`/api/v1/moments/${published.id}/comments`).send({ authorName: 'Guest', content: '' })).status).toBe(400)
