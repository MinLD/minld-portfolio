...
test('ADMIN can create list get update delete technology', async () => {
  ...
  const listed = await request(app).get('/api/v1/admin/technologies').set('Cookie', cookie)
  ...
  expect((await request(app).get(`/api/v1/admin/technologies/${id}`).set('Cookie', cookie)).status).toBe(200)
  ...
  expect((await request(app).delete(`/api/v1/admin/technologies/${id}`).set('Cookie', cookie)).status).toBe(204)
  expect((await request(app).get(`/api/v1/admin/technologies/${id}`).set('Cookie', cookie)).status).toBe(404)
})
...
test('technology admin routes require ADMIN', async () => {
  ...
  const user = await request(app).post('/api/v1/admin/technologies').set('Cookie', userCookie).send({ name: 'Test Technology User', slug: 'test-technology-user', type: 'TOOL' })
