...
test('ADMIN can create list get update delete category', async () => {
  ...
  const listed = await request(app).get('/api/v1/admin/categories').set('Cookie', cookie)
  ...
  expect((await request(app).get(`/api/v1/admin/categories/${id}`).set('Cookie', cookie)).status).toBe(200)
  ...
  expect((await request(app).delete(`/api/v1/admin/categories/${id}`).set('Cookie', cookie)).status).toBe(204)
  expect((await request(app).get(`/api/v1/admin/categories/${id}`).set('Cookie', cookie)).status).toBe(404)
})
...
test('category admin routes require ADMIN', async () => {
  ...
  const user = await request(app).post('/api/v1/admin/categories').set('Cookie', userCookie).send({ name: 'Test Category User', slug: 'test-category-user' })
...
test('category validation and uniqueness work', async () => {
  ...
  expect((await request(app).post('/api/v1/admin/categories').set('Cookie', cookie).send({ name: '', slug: 'bad slug' })).status).toBe(400)
  expect((await request(app).post('/api/v1/admin/categories').set('Cookie', cookie).send({ name: 'Test Category Unique', slug: 'test-category-unique' })).status).toBe(201)
  expect((await request(app).post('/api/v1/admin/categories').set('Cookie', cookie).send({ name: 'Test Category Unique', slug: 'test-category-unique-2' })).status).toBe(409)
  expect((await request(app).post('/api/v1/admin/categories').set('Cookie', cookie).send({ name: 'Test Category Unique 2', slug: 'test-category-unique' })).status).toBe(409)
})
